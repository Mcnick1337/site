...
// This string contains all the classes for our select dropdowns.
// It defines a light mode look, and then uses `dark:` prefixes to override it for dark mode.
const selectClasses = `
  w-full rounded-md border-0 py-1.5 shadow-sm
  focus:ring-2 focus:ring-inset focus:ring-cyan-500
  
  /* --- Light Mode Styles --- */
  appearance-auto /* Let the browser handle the appearance */
  bg-gray-200 text-gray-800 ring-1 ring-inset ring-gray-300

  /* --- Dark Mode Overrides --- */
  dark:appearance-none /* Remove browser appearance in dark mode */
  ...
  dark:bg-no-repeat
  dark:bg-[right_0.5rem_center]
  dark:bg-[length:1.5em_1.5em]
  dark:pr-10
`;
    ...
    return (
        <div className="bg-gray-100 dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-4 my-6 flex flex-wrap items-end gap-4">
            ...
            <div className="flex-grow min-w-[100px]">
                ...
                <select 
                    id={`type-filter-${modelId}`} value={filters.signalType} onChange={(e) => onFilterChange('signalType', e.target.value)} 
            ...
            <div className="flex-grow min-w-[100px]">
                ...
                <select 
                    id={`status-filter-${modelId}`} value={filters.status} onChange={(e) => onFilterChange('status', e.target.value)} 
                    className={selectClasses.replace(/\s+/g, ' ').trim()}>
            ...
            <div className="flex-grow min-w-[100px]">
                ...
                <select 
                    id={`sort-by-${modelId}`} value={sort.by} onChange={(e) => onSortChange('by', e.target.value)} 
                    className={selectClasses.replace(/\s+/g, ' ').trim()}>
