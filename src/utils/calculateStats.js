...
// --- REVERTED: Back to the day-by-day fetching logic to respect API limits ---
        ...
        try {
            ...
            for (const candle of ohlcData) {
                ...
                if (decision.toUpperCase() === 'LONG') {
                    if (high >= take_profit)
                        return { status: 'win', message: `Take Profit hit at ${new Date(candleTime).toLocaleString()}` };
                    if (low <= stop_loss)
                        return { status: 'loss', message: `Stop Loss hit at ${new Date(candleTime).toLocaleString()}` };
                } else { // SHORT
                    if (low <= take_profit)
                        return { status: 'win', message: `Take Profit hit at ${new Date(candleTime).toLocaleString()}` };
                    if (high >= stop_loss)
