...
export function calculateAllStats(signals) {
    ...
    const equityCurveData = [{ x: new Date(sortedSignals[0]?.timestamp || Date.now()).getTime(), y: equity, signalId: null }];
    const returns = [];
    ...
    sortedSignals.forEach(signal => {
        ...
        if (status === 'win' || status === 'loss') {
            ...
            if (!isNaN(entry) && !isNaN(sl) && !isNaN(tp1) && Math.abs(entry - sl) > 0) {
                ...
                totalRR += rr;
                rrCount++;
                const profitOrLoss = status === 'win' ? (100 * rr) : -100; // Assuming 1R = $100 for base equity
                if (status === 'win') {
                    grossProfit += profitOrLoss;
                    returns.push(rr);
                } else {
                    grossLoss += profitOrLoss;
                    returns.push(-1); // Assuming a 1R loss
                }
                equity += profitOrLoss;
            }
...
// Tier 5: Calculates equity curve based on compounding a portfolio.
export function calculateCompoundingEquityCurve(signals, initialCapital, riskPercent) {
    if (!signals || signals.length === 0) return [];
    
    let capital = initialCapital;
    const sortedSignals = [...signals].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    const equityCurve = [{ x: new Date(sortedSignals[0]?.timestamp || Date.now()).getTime(), y: capital }];
...
// Tier 1: Calculates performance broken down by symbol.
...
// Tier 1: Calculates performance broken down by time.
...
// Tier 5: Calculates the Pearson correlation coefficient between two models.
export function calculateCorrelation(signalsA, signalsB) {
    // 1. Create return streams for each model
    const returnsA = new Map(signalsA.filter(s => s.performance?.status).map(s => {
        ...
        return [s.timestamp, s.performance.status === 'WIN' ? (rr || 1) : -1];
    }));
    const returnsB = new Map(signalsB.filter(s => s.performance?.status).map(s => {
        const rr = Math.abs(parseFloat(s["Take Profit Targets"]?.[0]) - parseFloat(s["Entry Price"])) / Math.abs(parseFloat(s["Entry Price"]) - parseFloat(s["Stop Loss"]));
        return [s.timestamp, s.performance.status === 'WIN' ? (rr || 1) : -1];
    }));

    // 2. Find all unique timestamps from both models
    const allTimestamps = new Set([...returnsA.keys(), ...returnsB.keys()]);
    
    // 3. Create aligned arrays of returns
    let x = [];
    let y = [];
